...
const TransactionHistory = (props) => {
  const { _id, date, receiverId, amount } = props.transaction
  // console.log(props);
  
  const [transactionHistoryView, setTransactionHistoryView] = useState([]);

  useEffect(() => {
      fetch(`https://thawing-falls-79270.herokuapp.com/transaction/${_id}`)
        .then((res) => res.json())
      ...
      <div className='history-content-container'>
        ...
        >
          <div className='d-flex'>
            <div style={{textAlign: 'left', fontFamily: "Rajdhani"}}>
              <h6>Send</h6>
            </div>
